class Route
  ...
  def run(req, res)
    ...
    hash_params = Hash[ route_params.names.zip(route_params.captures) ]
    @controller_class.new(req, res, hash_params).invoke_action(@action_name)
  end
end
...
class Router
  ...
  def match(req)
    matched_routes = @routes.find do |route|
      route.matches?(req)
    end
  end
